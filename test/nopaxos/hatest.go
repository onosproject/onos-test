...
// TestNOPaxosHA : integration test
func (s *HATestSuite) TestNOPaxosHA(t *testing.T) {
	partitions := env.Database().Partitions("nopaxos")
	group, err := partitions.Connect()
	assert.NoError(t, err)
	assert.NotNil(t, group)

	m, err := group.GetMap(context.Background(), "TestNOPaxosHA", session.WithTimeout(5*time.Second))
	assert.NoError(t, err)

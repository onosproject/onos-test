...
// TestNOPaxosLock : integration test
func (s *SmokeTestSuite) TestNOPaxosLock(t *testing.T) {
	group, err := env.Database().Partitions("nopaxos").Connect()
	assert.NoError(t, err)
	assert.NotNil(t, group)

	lock1, err := group.GetLock(context.Background(), "TestNOPaxosLock", session.WithTimeout(5*time.Second))
	assert.NoError(t, err)

	lock2, err := group.GetLock(context.Background(), "TestNOPaxosLock", session.WithTimeout(5*time.Second))
	assert.NoError(t, err)

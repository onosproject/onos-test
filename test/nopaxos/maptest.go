...
// TestNOPaxosMap : integration test
func (s *SmokeTestSuite) TestNOPaxosMap(t *testing.T) {
	group, err := env.Database().Partitions("nopaxos").Connect()
	assert.NoError(t, err)
	assert.NotNil(t, group)

	m, err := group.GetMap(context.Background(), "TestNOPaxosMap", session.WithTimeout(5*time.Second))
	assert.NoError(t, err)
